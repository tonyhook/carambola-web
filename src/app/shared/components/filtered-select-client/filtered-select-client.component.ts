...
type FilteredSelectClientValue = Client | Client[] | null;
type FilteredSelectClientFormGroup = FormGroup<{
  selection: FormControl<FilteredSelectClientValue>;
...
@Component({
  ...
  templateUrl: './filtered-select-client.component.html',
  styleUrls: ['./filtered-select-client.component.scss'],
  providers: [
    {
      provide: MatFormFieldControl,
      useExisting: FilteredSelectClientComponent
    },
  ],
})
export class FilteredSelectClientComponent implements OnDestroy, ControlValueAccessor, MatFormFieldControl<Client | Client[]> {
  ...
  readonly _value = model<Client | Client[] | null>(null, {alias: 'value'});
  get value() {
    return this._value();
  }
  readonly stateChanges = new Subject<void>();
  readonly id = `filtered-select-client-${FilteredSelectClientComponent.nextId++}`;
  placeholder = '';
  focused = false;
  get empty() {
  ...
  _onChange: (arg0: Client | Client[] | null) => void = () => {return};
  _onTouched: () => void = () => {return};

  writeValue(value: Client | Client[] | null): void {
    this.changedByInternal = false;
    this.updateValue(value);
  }
  registerOnChange(fn: (arg0: Client | Client[] | null) => void): void {
    this._onChange = fn;
  }
  ...
  formGroup: FilteredSelectClientFormGroup;
  ...
  constructor() {
    ...
    this.formGroup = this.formBuilder.group({
      selection: this.formBuilder.control<FilteredSelectClientValue>(this.multiple() ? [] : null, this.required ? Validators.required : []),
    ...
    this.formGroup.valueChanges
      ...
      .subscribe(() => {
        ...
        if (Array.isArray(formSelection) && Array.isArray(selection)) {
          ...
          const b = new Set([...(selection as Client[]).map((item) => item.id)]);
  ...
  updateValue(value: Client | Client[] | null): void {
  ...
  filterCandidate() {
    ...
    if (!search) {
      ...
      this.filteredCandidates.set(this.options().filter(option => {
        ...
        if (option.name.indexOf(filter) < 0 && option.code.indexOf(filter) < 0) {
          return false;
        } else {
          return true;
        }
