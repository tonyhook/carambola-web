...
@Component({
  ...
  templateUrl: './filtered-select-vendor.component.html',
  styleUrls: ['./filtered-select-vendor.component.scss'],
  providers: [
    {
      provide: MatFormFieldControl,
      useExisting: FilteredSelectVendorComponent
    },
  ],
})
export class FilteredSelectVendorComponent implements OnDestroy, AfterViewInit, ControlValueAccessor, MatFormFieldControl<Vendor | Vendor[]> {
  ...
  readonly _value = model<Vendor | Vendor[] | null>(null, {alias: 'value'});
  get value() {
    return this._value();
  }
  readonly stateChanges = new Subject<void>();
  readonly id = `filtered-select-vendor-${FilteredSelectVendorComponent.nextId++}`;
  placeholder = '';
  focused = false;
  get empty() {
  ...
  _onChange: (arg0: Vendor | Vendor[] | null) => void = () => {return};
  _onTouched: () => void = () => {return};

  writeValue(value: Vendor | Vendor[] | null): void {
    this.changedByInternal = false;
    this.updateValue(value);
  }
  registerOnChange(fn: (arg0: Vendor | Vendor[] | null) => void): void {
    this._onChange = fn;
  }
  ...
  readonly filteredCandidates = signal<Vendor[]>([]);
  oldFilter = '';

  options = input<Vendor[]>([]);
  ...
  updateValue(value: Vendor | Vendor[] | null): void {
