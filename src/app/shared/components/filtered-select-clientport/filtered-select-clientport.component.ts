...
export class FilteredSelectClientPortComponent implements OnDestroy, ControlValueAccessor, MatFormFieldControl<ClientPort | ClientPort[]> {
  ...
  filteredCandidate$: ReplaySubject<ClientPort[]> = new ReplaySubject<ClientPort[]>(1);
  filteredCandidateCache: ClientPort[] = [];
  oldFilter = '';
    ...
    this.formGroup.valueChanges
      ...
      .subscribe((data) => {
        ...
        if (Array.isArray(data.selection) && Array.isArray(selection)) {
          const a = new Set([...(data.selection as ClientPort[]).map((item) => item.id)]);
