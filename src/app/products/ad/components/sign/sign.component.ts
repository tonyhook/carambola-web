...
import { AdQuery } from '../..';
import { BillAPI, BillView, Client, ClientAPI, ClientMedia, ClientMediaAPI, ClientPort, ClientPortAPI, ConnectionAPI, Medium, PartnerType, PerformanceAPI, PerformancePartner, PerformancePlaceholder, Sign, SignStatus, TimedPairedVendorPortMap, Vendor, VendorAPI, VendorMedia, VendorMediaAPI, VendorPort, VendorPortAPI } from '../..';
...
export class SignComponent implements OnInit, AfterViewInit, DoCheck {
  ...
  formQuery: AdQuery<PerformancePlaceholder> = {
    filter: {},
    searchKey: ['name', 'tagId'],
    searchValue: '',
  };
  formQuerySub: AdQuery<PerformancePlaceholder> = {
    filter: {},
    searchKey: ['name', 'tagId'],
    searchValue: '',
  };
  ...
  updateSignView() {
    ...
    if (this.signAggregateDownstream !== 'vendorport' || this.signInterval !== 'day' || this.signStatusFilter.indexOf(SignStatus.SIGN_STATUS_PENDING) >= 0) {
      for (const medium of this.mediumData) {
        ...
        signView.closed++;
        const vendorPortList = this.connectionData[new Date(medium.date).getTime()];
        if (vendorPortList) {
          const pairedClientPortList = vendorPortList[medium.vendorPort];
          if (pairedClientPortList && pairedClientPortList.length > 0) {
            signView.total += pairedClientPortList.length;
          } else {
            signView.total++;
          }
      ...
      for (const performance of performanceMap.values()) {
        ...
        if (!processed) {
          const vendorPortList = this.connectionData[new Date(performance.time).getTime()];
          if (vendorPortList) {
            const pairedClientPortList = vendorPortList[performance.vendorPort];
            if (pairedClientPortList && pairedClientPortList.length > 0) {
              signView.total += pairedClientPortList.length;
            } else {
              signView.total++;
        ...
        if (!processed || this.signViewTotal.request === null) {
          this.signViewTotal.request = this.signViewTotal.request === null ?
            performance.eventA + performance.eventB + performance.eventC + performance.eventD + performance.eventE +
            performance.eventF + performance.eventG + performance.eventH + performance.eventI + performance.eventJ
          : this.signViewTotal.request +
            performance.eventA + performance.eventB + performance.eventC + performance.eventD + performance.eventE +
            performance.eventF + performance.eventG + performance.eventH + performance.eventI + performance.eventJ;
        }
        if (!processed || this.signViewTotal.response === null) {
          this.signViewTotal.response = this.signViewTotal.response === null ?
            performance.eventI + performance.eventJ
          : this.signViewTotal.response +
            performance.eventI + performance.eventJ;
        }
        if (!processed || this.signViewTotal.request === null) {
  ...
  updateSignViewSub() {
    ...
    if (this.expandedRow) {
      const vendorPortList = this.connectionData[this.expandedRow.start.getTime()];
      if (vendorPortList) {
        for (const vendorPortValue of Object.keys(vendorPortList)) {
          const vendorPort = Number(vendorPortValue);
          if (this.expandedRow.vendor !== 0 && (this.vendorPortMap.get(vendorPort)?.vendor.id ?? 0) !== this.expandedRow.vendor) {
            continue;
          }
          if (this.expandedRow.vendorMedia !== 0 && (this.vendorPortMap.get(vendorPort)?.vendorMedia.id ?? 0) !== this.expandedRow.vendorMedia) {
            continue;
          }
          if (this.expandedRow.vendorPort !== 0 && vendorPort !== this.expandedRow.vendorPort) {
            continue;
          }

          const pairedClientPortList = vendorPortList[vendorPort];
          if (!pairedClientPortList) {
            continue;
          }

          for (const clientPort of pairedClientPortList) {
            if (this.expandedRow.client !== 0 && (this.clientPortMap.get(clientPort)?.client.id ?? 0) !== this.expandedRow.client) {
              continue;
            }
            if (this.expandedRow.clientMedia !== 0 && (this.clientPortMap.get(clientPort)?.clientMedia.id ?? 0) !== this.expandedRow.clientMedia) {
              continue;
            }
            if (this.expandedRow.clientPort !== 0 && clientPort !== this.expandedRow.clientPort) {
              continue;
            }

            let key = this.expandedRow.time + '|';
            ...
            if (!this.signViewMapSub.has(key)) {
              const signView: BillView = {
                time: this.expandedRow.time,
                start: new Date(this.expandedRow.start),
                end: new Date(this.expandedRow.start),
                client: this.signAggregateUpstream === 'client' || this.signAggregateUpstream === 'clientport' ? this.clientPortMap.get(clientPort)?.client.id ?? 0 : 0,
                clientMedia: this.signAggregateUpstream === 'clientport' ? this.clientPortMap.get(clientPort)?.clientMedia.id ?? 0 : 0,
                clientPort: this.signAggregateUpstream === 'clientport' ? clientPort : 0,
                vendor: this.signAggregateDownstream === 'vendor' || this.signAggregateDownstream === 'vendorport' ? this.vendorPortMap.get(vendorPort)?.vendor.id ?? 0 : 0,
                vendorMedia: this.signAggregateDownstream === 'vendorport' ? this.vendorPortMap.get(vendorPort)?.vendorMedia.id ?? 0 : 0,
                vendorPort: this.signAggregateDownstream === 'vendorport' ? vendorPort : 0,
                request: null,
                response: null,
                impression: null,
                click: null,
