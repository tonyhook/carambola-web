...
import { AdQuery } from '../..';
import { BillAPI, BillStatus, BillView, Client, ClientAPI, ClientMedia, ClientMediaAPI, ClientPort, ClientPortAPI, ConnectionAPI, Medium, PartnerType, PerformanceAPI, PerformancePartner, PerformancePlaceholder, PerformanceView, PortType, Sign, TimedPairedVendorPortMap, Vendor, VendorAPI, VendorMedia, VendorMediaAPI, VendorPort, VendorPortAPI } from '../..';
import { TenantService } from '../..';
import { AdEntityComponent, FilteredSelectClientComponent, FilteredSelectClientMediaComponent, FilteredSelectVendorComponent, FilteredSelectVendorMediaComponent } from '../..';
...
export class SummaryComponent implements OnInit, AfterViewInit, DoCheck {
  ...
  formQueryUpstream: AdQuery<PerformancePlaceholder> = {
    filter: {},
    searchKey: ['name', 'tagId'],
    searchValue: '',
  };
  formQueryDownstream: AdQuery<PerformancePlaceholder> = {
    filter: {},
    searchKey: ['name', 'tagId'],
    searchValue: '',
  };
  ...
  dataRequest$ = new Subject<[AdQuery<PerformancePlaceholder>, AdQuery<PerformancePlaceholder>]>();
  ...
  downloadTemplate() {
    const a = document.createElement('a');
    a.href = 'assets/ad/template.xlsx';
  ...
  updateSummaryView() {
    ...
    const formQueryUpstream: AdQuery<ClientPort> = {
      filter: {
        clientMode: [String(this.mode())],
        vendorMode: [String(this.mode())],
        client: this.selectedClients.map(client => client.id!.toString()),
    ...
    const formQueryDownstream: AdQuery<VendorPort> = {
      filter: {
        clientMode: [String(this.mode())],
        vendorMode: [String(this.mode())],
        vendor: this.selectedVendors.map(vendor => vendor.id!.toString()),
      ...
      for (const performance of this.performanceData) {
        ...
        if (this.mode() === PartnerType.PARTNER_TYPE_PROGRAMMATIC) {
          ...
          if (this.clientPortMap.get(performance.clientPort)?.mode === PortType.PORT_TYPE_SHARE) {
            const mediumKey = this.toISOStringWithTimezone(new Date(performance.time)).substring(0, 10).replace('T', ' ') + '|' + performance.clientPort + '|' + performance.vendorPort;
            performanceView.income += this.mediumUpstreamMap.get(mediumKey)?.income ?? 0;
          }
          performanceView.outcomeUpstream += performance.outcomeUpstream;
          performanceView.outcomeRebate += performance.outcomeRebate;
          performanceView.outcomeDownstream += performance.outcomeDownstream;
          ...
          if (this.clientPortMap.get(performance.clientPort)?.mode === PortType.PORT_TYPE_SHARE) {
            const mediumKey = this.toISOStringWithTimezone(new Date(performance.time)).substring(0, 10).replace('T', ' ') + '|' + performance.clientPort + '|' + performance.vendorPort;
            this.performanceViewTotal.income += this.mediumUpstreamMap.get(mediumKey)?.income ?? 0;
          }
