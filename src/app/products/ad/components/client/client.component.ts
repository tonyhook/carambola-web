...
import { AdQuery } from '../..';
import { Client, ClientAPI, PartnerType } from '../..';
...
export class ClientManagerComponent implements OnInit, AfterViewInit {
  ...
  dataRequest$ = new Subject<AdQuery<Client>>();
  ...
  ngOnInit() {
    this.dataRequest$.pipe(
      ...
      const data = clients.filter(client => !client.deleted).sort((a, b) => {
  ...
  private createDataSource(data: Client[]): MatTableDataSource<Client> {
